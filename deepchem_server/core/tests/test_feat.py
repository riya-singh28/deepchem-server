...
def test_featurize_multicore_restart(disk_datastore):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    dataset_path = os.path.join(current_dir, "assets/zinc5k.csv")
    partial_checkpoints_dir_path = os.path.join(current_dir,
                                                "assets/feat_test.partial")
    df = pd.read_csv(dataset_path)
    card = DataCard(address='', file_type='csv', data_type='pandas.DataFrame')
    data_address = disk_datastore.upload_data_from_memory(df, "zinc.csv", card)
    ...
    shutil.copytree(partial_checkpoints_dir_path,
                    disk_datastore.storage_loc + "/" + checkpoint_output_key)
...
def test_featurize_multicore_sdf(disk_datastore):
    ...
    data_address = disk_datastore.upload_data("membrane.sdf", dataset_path,
                                              card)
    dataset_address = featurize(data_address,
                                featurizer="ecfp",
...
def test_featurize_sdf(disk_datastore):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    sdf_test_file = os.path.join(current_dir,
                                 "assets/membrane_permeability.sdf")

    data_address = disk_datastore.upload_data("membrane.sdf", sdf_test_file,
                                              card_sdf)
    dataset_address = featurize(data_address,
                                featurizer="ecfp",
                                output="feat_test",
...
def test_molgraphconv_feat(disk_datastore):
    ...
    """
    df = pd.DataFrame(["C1=CC=CN=C1", "O=C(NCc1cc(OC)c(O)cc1)CCCC/C=C/C(C)C"],
                      columns=["smiles"])
    config.set_datastore(disk_datastore)
    card = DataCard(address='', file_type='csv', data_type='pandas.DataFrame')
    data_address = disk_datastore.upload_data_from_memory(df, "test.csv", card)
